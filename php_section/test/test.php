...
for($k=0; $k<500; $k++){
	for($i=0;$i<100000;$i++){
		$hash1=hash("sha256", $i."sgsdflkhjfgjsgfhegrjhsegr");
		$res.= ($k*100000+$i)."\t".$hash1."\n";
		$hash2 = hash("sha256", $hash1);
		$res2.=($k*100000+$i)."\t".$hash2."\n";;
	}
	fp("G:/prog/keys/part$k.k",$res);
	fp("G:/prog/keys2/part$k.k",$res2);
	$res="";
	$res2="";
}
// echo $crypt->hash("test");
// echo $crypt->verify("test", $crypt->hash("test"));
exit;
echo preg_match("#(.*)void.megawall.ru#", "qwe.voi1d.megawall.ru");
exit;
echo preg_replace("#http://(?!megawall.ru)#","http://megawall.ru/redirect?loc=http://\\1","http://megawall.ru/");
exit;
...
// 	$value = $ar[$i];
// 	$var = explode("-", $value);
// 	$fromip = ip2long($var[0]);
// 	$toip = ip2long($var[1]);
// 	$res[floor($i/10000)].="\"$fromip\",\"$toip\",\"RU\",,\n";
// }
...
//  exit;
echo get_country_code_by_ip("93.100.152.70");
echo 1;
//fp("log.txt", $_SERVER[REQUEST_URI]."\n".$_SERVER['HTTP_USER_AGENT']."\n".$_SERVER[http_referer]."\n",true);
//print_r(get_random_page_info(100000, 1));
exit;
add_pseudo_user(1);
//encoding_fix();
